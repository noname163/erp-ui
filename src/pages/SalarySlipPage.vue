...
const employeeOptions = [
  { value: 'EMP1001', label: 'John Doe - EMP1001' },
  { value: 'EMP1002', label: 'Jane Smith - EMP1002' },
  { value: 'EMP1003', label: 'Michael Scott - EMP1003' },
]
  ...
  <AppLayout>
    <div class="max-w-5xl mx-auto px-2 md:px-0 py-2 md:py-0">
      <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-black leading-tight tracking-tight text-slate-900 dark:text-white">Employee Salaries Entry</h1>
        ...
        <div class="flex items-center gap-3">
          <UiButton variant="outline" leadingIcon="close" :disabled="loading" @click="resetForm">Cancel</UiButton>
          <UiButton variant="primary" leadingIcon="save" :disabled="loading" @click="createSlip">Save Entry</UiButton>
        </div>
      </div>
      ...
      <div class="ui-card mb-8">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div class="lg:col-span-2">
            <UiSelect
              v-model="master.userProfileCode"
              label="Employee"
              required
              placeholder="Select Employee (Name / ID)"
          ...
          <UiInput :model-value="totalAmount" label="Total Amount (Auto)" disabled hint="Auto-calculated from salary details" />
        </div>
      </div>
      ...
      <div class="ui-card">
        <div class="border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
          ...
          <UiButton variant="outline" leadingIcon="add" :disabled="loading" @click="addDetail">Add New Row</UiButton>
        </div>
        ...
        <UiTable
          ...
          :rows="details"
          :row-key="(_, i) => i"
          head-class="bg-slate-50 dark:bg-slate-900/50"
          body-class="divide-slate-100 dark:divide-slate-800"
        ...
        >
          <template #cell-salaryCode="{ row, index }">
            <UiInlineSelect
              v-model="details[index].salaryCode"
              :options="salaryCodeOptions"
          ...
          <template #cell-remark="{ index }">
            <UiInlineInput v-model="details[index].remark" type="text" placeholder="Add specific details..." />
          </template>

          <template #cell-action="{ index }">
            <button class="text-slate-400 hover:text-red-500 transition-colors" type="button" @click="removeDetail(index)">
      ...
      <div class="mt-8 flex flex-col sm:flex-row items-center justify-between p-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div class="flex items-center gap-3">
          <UiButton variant="outline" leadingIcon="description" :disabled="loading" @click="saveDraft">Save Draft</UiButton>
          <UiButton variant="primary" leadingIcon="send" :disabled="loading || !canSubmit" @click="createSlip">
            Submit Final Entry
          </UiButton>
        </div>
      </div>
      ...
      <div class="mt-6">
        <UiBadge variant="info" icon="info">Amounts are summed as strings to avoid floating point issues.</UiBadge>
      </div>
    </div>
  </AppLayout>
</template>
