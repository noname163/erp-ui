...
async function submit() {
  ...
  try {
    ...
    message.value = typeof res === 'string' ? res : 'Bulk daily work created'
  } catch (e: any) {
    error.value = e?.message ?? 'Invalid JSON or request failed'
  } finally {
    loading.value = false
  }
}
...
<template>
  <AppLayout>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold">Bulk Log Daily Work</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">POST /api/employee-daily-works</p>
      </div>
      <UiButton variant="primary" :disabled="loading" @click="submit">Submit</UiButton>
    </div>
    ...
    <div class="ui-card">
      <div class="p-4 md:p-6">
        ...
        <label class="block text-sm font-medium mb-2">Payload (JSON array)</label>
