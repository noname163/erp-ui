...
<template>
  <div class="bg-slate-50 dark:bg-background-dark text-slate-900 dark:text-slate-100 overflow-hidden min-h-screen">
    <div class="flex min-h-screen w-full">
      <div class="w-full lg:w-[480px] flex flex-col justify-center px-8 lg:px-16 bg-white dark:bg-slate-900 z-10 shadow-xl relative">
        <div class="absolute top-6 right-8 lg:right-16">
          <LocaleSwitcher />
        </div>
        <div class="mb-10">
          <div class="flex items-center gap-3 mb-10">
            ...
            <span class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{{ t('login.brand') }}</span>
          </div>

          <h1 class="text-h1 text-slate-900 dark:text-white mb-3">{{ t('login.title') }}</h1>
        ...
        <form class="space-y-6" @submit.prevent="submit">
          <div>
            <p class="ui-label mb-1.5 text-slate-700 dark:text-slate-200">{{ t('login.emailLabel') }}</p>
            <UiInput v-model="email" type="email" :placeholder="t('login.emailPlaceholder')" required autocomplete="username" name="email" />
          </div>

          <div>
            <div class="flex justify-between items-center mb-1.5">
              <p class="ui-label text-slate-700 dark:text-slate-200">{{ t('login.passwordLabel') }}</p>
              <button
                type="button"
                class="text-xs font-semibold text-primary hover:underline transition-colors"
                @click="error = t('login.forgotPasswordNotImplemented')"
              >
          ...
          <div class="flex items-center">
            <input id="remember" v-model="remember" type="checkbox" class="w-4 h-4 text-primary border-slate-300 rounded focus:ring-primary" />
          ...
          <UiButton type="submit" variant="primary" block :disabled="loading" class="h-12" trailingIcon="login">
            <span class="text-button">{{ loading ? t('login.signingIn') : t('login.signIn') }}</span>
          </UiButton>
        </form>
        ...
        <div class="mt-12 pt-8 border-t border-slate-100 dark:border-slate-800">
          <div class="flex flex-col gap-4">
            ...
            <div class="flex items-center gap-4">
              <UiBadge variant="info" icon="security">{{ t('login.sslEncrypted') }}</UiBadge>
              <UiBadge variant="info" icon="verified_user">{{ t('login.mfaEnabled') }}</UiBadge>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="hidden lg:flex flex-1 relative bg-slate-900 overflow-hidden">
        ...
        <div class="relative z-20 flex flex-col justify-between h-full p-16 text-white">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold uppercase tracking-[0.2em] text-white/80">{{ t('login.portalTag') }}</span>
          </div>
          <div>
            <h2 class="text-5xl font-bold leading-[1.15] mb-6 tracking-tight">
              {{ t('login.heroTitleLine1') }} <br />
              <span class="text-white/60">{{ t('login.heroTitleLine2') }}</span>
            </h2>
            <p class="text-xl text-white/80 max-w-xl leading-relaxed">{{ t('login.heroDescription') }}</p>
          </div>
          <div class="flex items-center justify-between">
            ...
            <p class="text-xs text-white/50 tracking-wide">{{ t('login.copyright') }}</p>
          </div>
        </div>
