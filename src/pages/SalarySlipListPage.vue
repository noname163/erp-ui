...
async function loadRows() {
  ...
  try {
    const res = await employeeSalaryService.list({
      employeeName: isEmployeeRole.value ? undefined : applied.value.search || undefined,
...
function formatMoney(amount: number, currency: string) {
  ...
  try {
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: resolvedCurrency, minimumFractionDigits: 2 }).format(amount)
  } catch {
...
const pendingReviews = computed(() => filteredRows.value.filter((row) => row.totalAmount >= 90000).length)
  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
          <UiButton variant="outline" leading-icon="filter_list" @click="toggleFilters">
            {{ isFilterOpen ? t('salarySlips.list.actions.hideFilters') : t('salarySlips.list.actions.showFilters') }}
          </UiButton>
          <UiButton variant="outline" leading-icon="download" @click="exportCsv">{{ t('common.action.export') }}</UiButton>
          <UiButton
            v-if="!isEmployeeRole"
            variant="primary"
            leading-icon="add_circle"
      ...
      <UiCard v-if="isFilterOpen">
        <UiCardBody>
          <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 items-end">
            <div v-if="!isEmployeeRole" class="lg:col-span-2">
              <UiInput
                v-model="search"
                :label="t('salarySlips.list.filters.employeeOrCurrency')"
                :placeholder="t('salarySlips.list.filters.employeeOrCurrencyPlaceholder')"
                leading-icon="search"
                @keyup.enter="applyFilters"
              />
            </div>

            <div :class="isEmployeeRole ? 'lg:col-span-3' : 'lg:col-span-2'">
            ...
            <div :class="isEmployeeRole ? 'lg:col-span-2' : ''">
          ...
          <div class="flex justify-end gap-2 mt-4">
            ...
            <UiButton variant="primary" leading-icon="filter_alt" @click="applyFilters">
              {{ t('salarySlips.list.actions.applyFilters') }}
            </UiButton>
          </div>
        </UiCardBody>
      </UiCard>
      ...
      <UiCard>
        <div class="p-4 md:p-6">
          ...
          >
            ...
            <template #cell-effectiveTo="{ row }">
              <span :class="row.effectiveTo ? 'text-slate-600 dark:text-slate-300' : 'italic text-slate-500'">
                {{ formatDate(row.effectiveTo) }}
              </span>
            </template>

            <template #cell-totalAmount="{ row }">
              <span class="font-bold text-slate-900 dark:text-white">{{ formatMoney(row.totalAmount, row.currency) }}</span>
            </template>
        ...
        <div class="px-4 md:px-6 py-4 border-t border-primary/10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <UiButton variant="outline" :disabled="currentPage >= totalPages" @click="setPage(currentPage + 1)">
        ...
        <UiCard>
          <UiCardBody>
            <div class="flex items-center gap-4">
              <div class="size-12 rounded-lg bg-green-500/10 text-green-500 flex items-center justify-center">
              ...
              <div>
                ...
                <p class="text-xl font-black text-slate-900 dark:text-white">{{ formatMoney(totalPayroll, 'USD') }}</p>
              </div>
            </div>
          </UiCardBody>
        </UiCard>
